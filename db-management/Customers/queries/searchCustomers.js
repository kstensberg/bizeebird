...
const searchCustomers = (db, searchString) => {
    return new Promise((resolve, reject) => {
        db.serialize(() => {
            ...
            var query = 'SELECT Customers.Name, CustomerPhoneNumbers.PhoneNumber AS PhoneNumber, Customers.Email, Customers.BoardingRate, Customers.Notes FROM Customers LEFT JOIN CustomerPhoneNumbers ON CustomerPhoneNumbers.Customer_CustomerId = Customers.CustomerId WHERE Customers.Email LIKE ? OR Customers.Name LIKE ? OR CustomerPhoneNumbers.PhoneNumber LIKE ? GROUP BY Customers.Name';
